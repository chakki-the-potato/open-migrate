...
## 테스트 모드에서의 `~/.claude.json`

`~/.claude.json` 은 `~/.claude` 디렉토리 **바깥**에 있다. 테스트 모드에서의 대응 위치는 procedure.md "테스트 모드의 경로 해석" 규칙대로 **`<루트>/.claude.json`** 이며, 실제 `~/.claude.json` 은 읽지 않는다 — security.md 가 그 파일의 나머지 키를 읽지 말라고 한 것과도 맞물린다.
