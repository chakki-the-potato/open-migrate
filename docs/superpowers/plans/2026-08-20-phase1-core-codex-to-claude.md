...
Expected: `SKILL.md`, `core/` 가 보이고 `core/tools/` 에 codex.md, claude.md 존재. `_template.md` 는 저작용 템플릿이라 설치본에서 제외된다.
...
Expected: 중복 병합 없음, 2회차 리포트에 "이미 이관됨" 표기, 검증기는 재실행 후에도 전 항목 PASS.

검증기는 백업·MCP 명령 같은 실행 단위 산출물을 최신 run 하나가 아니라 **모든 run 을 통틀어** 찾는다. 원장 스킵으로 아무것도 바꾸지 않은 재실행은 백업도 새 MCP 명령도 남기지 않는 것이 정상이기 때문이다.
