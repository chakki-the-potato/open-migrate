...
- 파일이 여러 개면 파일별로 구분해 순서대로 병합한다. 구분은 소스 파일명을 담은 하위 헤딩(`### <파일명>`)으로 하고, 타겟 쪽 병합 섹션 헤딩(`## Migrated from ...`)보다 한 단계 아래를 쓴다. 소스 본문에 더 상위 레벨 헤딩(`# ...`)이 있어도 **본문은 원문 그대로 두고 레벨을 조정하지 않는다** — 헤딩 레벨 역전은 허용되며 원문 보존이 우선이다.
